...
项目统一使用 SQLite 内存数据库进行测试，具有以下优势：

- **无需配置**：不需要创建测试数据库或配置连接信息
- **速度快**：内存数据库性能优异，测试运行更快
- **隔离性好**：每个测试函数独立的数据库实例，互不干扰
- **自动清理**：测试结束后自动清理，无需手动维护

测试框架会在 `tests/integration/conftest.py` 中自动创建和删除表，开发者无需关心数据库管理。
